...
On **Navidrome 0.64+** a **Refresh** row sits under Limit: a free-text duration (`90m`, `12h`, `1d`, `1w`) setting how long the server waits before re-evaluating *this* playlist. Empty means the server's own interval — Navidrome's web UI has no control for it at all. The row appears only when your server supports it or the rules already carry a delay; see [Sort, limit, and refresh](/reference/smart-playlist-rules/#sort-limit-and-refresh) for the accepted spellings.
