...
Because Nokkvi keeps no disk cache for browsing artwork, your Navidrome server does the heavy lifting of serving and resizing artwork. Two settings in `navidrome.toml` are worth tuning — both are documented in Navidrome's [available options reference](https://www.navidrome.org/docs/usage/configuration/options/#available-options):
