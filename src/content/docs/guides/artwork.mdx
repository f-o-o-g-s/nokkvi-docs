...
| Mode | Behavior |
| :--- | :------- |
| `auto` *(default)* | Width is derived from the window — up to 40% of window width or 1000 px, whichever is smaller. The column hides automatically when the remaining slot-list width would drop below 800 px. Portrait-shaped windows scale up to 60% of width instead, capped by window height. |
...
Both `always_*` modes show a thin drag handle between the slot list and the artwork column. Drag horizontally to resize; the new fraction is saved to [`artwork_column_width_pct`](/reference/config/#artwork-column) (clamped to 0.05–0.80). `auto` sizes itself, so there's no handle.
