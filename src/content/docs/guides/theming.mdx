...
All keys have defaults — you only need to specify the ones you want to override. Omitted keys fall back to a built-in GNOME-blue palette (shown in the table below).
...
You don't need to specify every key. Any omitted key falls back to the built-in GNOME-blue defaults. Here's a theme that only changes the accent color:
