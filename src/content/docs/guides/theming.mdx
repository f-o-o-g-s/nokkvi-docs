...
Nokkvi comes with 22 built-in themes, but you can easily create your own using simple TOML files.

## Built-in themes

Switch themes from the **Settings** menu (default: `` ` `` backtick). Here's a sampler of the 22 built-ins — every frame is the same Queue view, so the only thing changing is the theme's background, accent (the focused row + progress bar), text, and visualizer gradient.
...
Any `.toml` file placed in this directory will automatically appear in the **Settings** menu (default: `` ` `` backtick). Built-in themes are seeded to this directory on first launch, so you can open any of them as a starting point. Fresh installs start on **Svalbard** (Nokkvi's default theme); switch from the Settings menu at any time.
...
Editable in Settings → Theme → **Chrome Border**; the row has a one-click reset back to the derived default.
