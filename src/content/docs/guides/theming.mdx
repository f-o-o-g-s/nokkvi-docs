...
Switch themes from the **Settings** menu (default: `` ` `` backtick). Here's a sampler of the 23 built-ins — every frame is the same Queue view, so the only thing changing is the theme's background, accent (the focused row + progress bar), text, and visualizer gradient.
