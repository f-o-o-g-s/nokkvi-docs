...
To uninstall: `yay -Rns nokkvi-bin` (or `nokkvi-git`). Per-user files are left in place — remove them manually with `rm -rf ~/.config/nokkvi/ ~/.local/state/nokkvi/` if you want a full wipe. Config (`config.toml`, `themes/`, `sfx/`) lives in `~/.config/nokkvi/`; runtime state (`app.redb`, `nokkvi.log`) lives in `~/.local/state/nokkvi/`.
...
Nokkvi is GUI-first; the CLI surface is intentionally tiny:

```bash
nokkvi -V, --version    # print version and exit
nokkvi -h, --help       # print usage, env vars, and config/state file paths
```

Logging is controlled via `RUST_LOG` (e.g. `RUST_LOG=debug nokkvi`). Run `nokkvi --help` for the full list of recognized values.
