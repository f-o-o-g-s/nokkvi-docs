...
To uninstall: `yay -Rns nokkvi-bin` (or `nokkvi-git`). Per-user files are left in place — remove them manually with `rm -rf ~/.config/nokkvi/ ~/.local/state/nokkvi/ ~/.cache/nokkvi/` if you want a full wipe. Config (`config.toml`, `themes/`, `sfx/`) lives in `~/.config/nokkvi/`; runtime state (`app.redb`, `nokkvi.log`) lives in `~/.local/state/nokkvi/`; MPRIS album-art (`mpris-art-<pid>.jpg`) lives in `~/.cache/nokkvi/`.
