...
A row's right-click menu also carries **Remove Duplicates**, which shrinks the queue to one row per song. It keeps the first copy of each — except the song that's playing, where the row under the play cursor is the keeper and even an *earlier* copy goes. That's what lets the sweep run without touching playback: the row you're hearing is never the one removed. A toast reports the result — *Removed 3 duplicates*, or *No duplicates found*.
...
This cover is also where [lyrics](/guides/lyrics/) display when the Lyrics toggle is on, synced ones following the track line by line and plain ones drifting with playback — and where the over-cover [visualizer](/reference/visualizer/) draws. All three coexist: the visualizer keeps playing beneath the words.
