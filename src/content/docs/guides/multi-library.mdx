...
The popover header shows `Active Libraries` on the left with an `N / M` count on the right, and each row shows the library name plus its song count (when the server exposes one).
