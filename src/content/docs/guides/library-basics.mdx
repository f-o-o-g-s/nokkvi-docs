...
The strip at the bottom of every view is fixed-height and shared across all views: a three-button transport — previous / play-pause / next — on the left, the progress bar in the middle, and mode toggles (repeat, shuffle, consume, equalizer, sound effects, crossfade, bit-perfect, visualizer, lyrics) plus volume sliders on the right. Stop isn't a transport button; it's on the media keys and `nokkvi stop`. See [Keyboard Shortcuts](/reference/hotkeys/) for the transport key bindings, [Audio](/guides/audio/) for what the mode toggles do, and [Lyrics](/guides/lyrics/) for the lyrics overlay.
