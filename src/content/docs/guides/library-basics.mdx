...
![The player bar at the bottom of every view — now-playing artwork and metadata on the left, transport in the middle, mode toggles and volume on the right, under a full-width progress scrub](../../../assets/player-bar.webp)

The strip at the bottom of every view is fixed-height and shared across all views. In the default mini-player layout it carries the now-playing artwork and metadata on the left, a three-button transport — previous / play-pause / next — in the middle, and mode toggles (repeat, shuffle, consume, equalizer, sound effects, crossfade, bit-perfect, visualizer, lyrics) plus volume sliders on the right, under a full-width progress scrub. [Track Info Display](/reference/track-info-display/) covers the other layouts, including the classic bar with the transport on the left and the progress bar in the middle. Stop isn't a transport button; it's on the media keys and `nokkvi stop`. See [Keyboard Shortcuts](/reference/hotkeys/) for the transport key bindings, [Audio](/guides/audio/) for what the mode toggles do, and [Lyrics](/guides/lyrics/) for the lyrics overlay.
...
| Key | Action |
| :-- | :----- |
| `Shift + Left` | Previous sort mode |
| `Shift + Right` | Next sort mode |
| `Page Up` | Toggle ascending / descending |

The bare arrows seek the playing track instead — see [Seek Backward / Seek Forward](/reference/hotkeys/#playback).
...
With `slot_text_links` enabled (off by default), many views render the artist, album, or genre as clickable text on each row. Clicking those values jumps you to the corresponding view and expands the target inline at the top, so its contents are immediately visible — no follow-up `Shift + Enter` needed. For example, clicking an artist name on a song row takes you to the Artists view with that artist's albums already expanded below the row.
...
| **Add to Mix** | Drop the focused item (or selection) into the [Trawl](/guides/trawl/) crate as a seed. Available on Albums, Artists, Songs, Genres, Playlists, the Similar tab, and the Queue |
...
**Add to Playlist** only lists manual playlists as targets: [smart playlists](/guides/smart-playlists/) are rule-driven, so their tracks are read-only. For the same reason, expanding a smart playlist shows its tracks without a **Remove from Playlist** action.
