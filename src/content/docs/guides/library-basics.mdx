...
Every slot-list view's sort dropdown ends with a **Roulette** entry. Picking it spins the list like a slot machine for ~5 seconds — the wheel decelerates with an ease-out curve plus a small fake-out near the end, the per-frame Tab navigation SFX tracks the slowing wheel for the slot-machine click, and the run lands on a random item which is then auto-played. The fake-out shape and timing vary every spin — patterns include none, a single near-miss, a two-step walk, an overshoot bounce, and a zigzag — and roughly one in five spins skips the fake-out entirely.
...
**Cancel** by pressing `Escape` during the spin or switching views — the original viewport is restored and nothing auto-plays. Roulette is disabled inside the [Library Browser](/guides/queue/#library-browser-and-drag-and-drop) side panel because play actions there route to add-to-queue instead.
