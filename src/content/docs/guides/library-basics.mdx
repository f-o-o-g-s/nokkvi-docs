...
The strip at the bottom of every view is fixed-height and shared across all views: previous / play / pause / stop / next on the left, the progress bar with an animated waveform visualizer in the middle, and mode toggles (repeat, shuffle, crossfade, equalizer, sound effects, visualizer) plus volume sliders on the right. See [Keyboard Shortcuts](/reference/hotkeys/) for the transport key bindings and [Audio](/guides/audio/) for what the mode toggles do.
