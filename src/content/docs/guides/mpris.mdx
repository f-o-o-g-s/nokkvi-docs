...
For verbs MPRIS doesn't expose (`switch-view`, `love`, `rate`, `clear-queue`, `consume`, `queue-push`, `queue-pull`), see the [`nokkvi` CLI](/guides/cli/).
...
| `Raise` | `CanRaise` is true. Reopens a window closed to the tray, or flags an open one for attention; the same action as [`nokkvi show`](/guides/cli/#misc). | |
...
| `mpris:artUrl` | Published as a `file://` URI pointing to `$XDG_CACHE_HOME/nokkvi/mpris-art-<pid>-<cover_id>_<hash>.jpg`. Nokkvi writes a new file per cover so desktop notifications never reuse a stale image. See [What art gets published](#what-art-gets-published). |  |

## What art gets published

Desktop shells keep showing the last image when `mpris:artUrl` is missing, so Nokkvi always publishes the key while something is current — otherwise a radio station would sit under the previous queue song's cover. What it points at, in order:

- **A song** — its cover, fetched once per track change; if that fetch fails, the Nokkvi icon.
- **A radio station** — the station's uploaded logo; failing that, the stream's ICY `StreamUrl` when it's an `http(s)` address (published verbatim, never fetched); failing that, the Nokkvi icon.

The icon placeholder is written once per process as `mpris-art-<pid>-placeholder.jpg`. **Not Playing** publishes no art at all. With no writable cache directory only a stream URL can go out, and a failed write is retried after a minute rather than on every tick.
