...
`Shift + A` and `Ctrl + Enter` are the two [mix actions](#playing-the-mix) in key form — append versus replace. Press either with an empty crate and Trawl says *The crate is empty — add seeds first* instead of doing nothing.
...
| Key | Action |
| :-- | :----- |
| `Shift + Tab` | Step the ring forward — **Blend → Minimum length → Maximum length → Minimum rating → Max tracks**, then back to nothing focused |
