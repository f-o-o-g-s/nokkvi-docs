...
![The Trawl modal: a crate of four seeds (two genres, an album, and an artist) in the tray, the whole-library search adding the artist, the Interleave/Weighted/Shuffle-all blend pills, the length, rating, and max-tracks filters, and the Clear / Add to Queue / Play Mix actions](../../../assets/trawl.webp)
...
| Right-click → **Add to Mix** | On rows in Albums, Artists, Songs, Genres, Playlists, the Similar tab, and the Queue |
...
| **Rating** | Any rating · Unrated only · 1 star and up … 5 stars only | Any rating |
...
| **Add to Queue** | `Shift + A` | Blends the crate and **appends** it to the current queue; the modal stays open |
| **Play Mix** | `Ctrl + Enter` | Blends the crate, **replaces** the queue with it, starts from the first track, and jumps to the Queue view |
| **Save as Playlist** | `Shift + P` | Resolves the mix to concrete songs, then opens a **Save Mix as Playlist** name dialog |
...
**Save as Playlist** freezes the mix: it resolves the blend to a fixed list of songs and saves them as an ordinary [playlist](/guides/playlists/) that *won't* re-evaluate (the dialog says as much — *Saves these N songs as an ordinary playlist — it won't update*). That's the difference from a [smart playlist](/guides/smart-playlists/), which stays live. It's not a way to save the crate itself — the crate is a scratch pad and still clears on logout.
...
| `Shift + A` | Add the whole mix to the queue — the keyboard twin of the **Add to Queue** button (append) |
| `Ctrl + Enter` | Play the mix — the keyboard twin of **Play Mix** (replaces the queue) |
| `Shift + P` | Save the mix as a playlist — the keyboard twin of **Save as Playlist** |
...
`Shift + A`, `Ctrl + Enter`, and `Shift + P` are the [mix actions](#playing-the-mix) in key form — append, replace, and save. Press any of them with an empty crate and Trawl says *The crate is empty — add seeds first* instead of doing nothing.

### Editing the controls from the keyboard
...
| Key | Action |
| :-- | :----- |
| `Shift + Tab` | Step the ring forward — **Blend → Minimum length → Maximum length → Rating → Max tracks**, then back to nothing focused |
| `Shift + Backspace` | Step the ring backward through the same controls |
| `Left` | Step the focused control to its previous value (wraps around) |
| `Right` | Step the focused control to its next value (wraps around) |
