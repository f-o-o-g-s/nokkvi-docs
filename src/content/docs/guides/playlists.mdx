...
Each playlist row shows its cover art — a 2×2 quad of the first distinct album covers — and its name. Song count and duration are optional columns (off by default; toggle them from the columns gear, or they auto-show when you sort by that field). The large artwork panel for the centered playlist shows a 3×3 collage for multi-album playlists, with a song count / duration / Updated overlay (`playlists_artwork_overlay`, on by default). Search matches playlist name and comment. Private playlists carry a small lock icon next to the name; public ones render without it.

[Smart playlists](/guides/smart-playlists/) — rule-based playlists the server keeps up to date — live in this list too, marked with a small **sparkles** badge (tooltip *Smart playlist — updates itself from rules*). They browse and play like any other playlist; where they differ is in editing, covered on their own page.
...
1. **`+` button — Playlists view header** — drops you straight into a blank [editor](#editing-playlists) where you type the name and add tracks. There's no naming dialog first: the playlist isn't written to the server until your first Save (an empty name blocks it, and a double-click can't mint a duplicate). On a server that supports [smart playlists](/guides/smart-playlists/), the `+` is instead a small menu — **New Playlist** / **New Smart Playlist** / **Import .nsp File**.
...
![The pin chip in the Playlists view header, hovered — its tooltip reads "Default Playlist: (none) — click to set" because nothing is pinned yet](../../../assets/default-playlist-chip.webp)

### The picker

A searchable modal listing every manual playlist with its artwork, song count, and duration ([smart playlists](/guides/smart-playlists/) can't be an append target, so they're left out):
...
`quick_add_to_playlist` (Settings → Playback → **Quick Add to Playlist**) makes the right-click **Add to Playlist** action skip its dialog. With it on and a default set, songs append straight to the default and a toast confirms how many were added. With it off, or with no default configured, the dialog opens as usual.
...
## Songs the playlist already has

Before anything is written, Nokkvi reads the target's track list and checks your batch against it. If none of the songs are there, the add just happens — *Added 3 songs to 'Road Trip'*. If some are, nothing is written yet and a confirm opens instead:

> **2 of 5 songs are already in "Road Trip".**
> ☑ Skip the 2 already there

The box is **checked by default**, so the primary button reads **Add** and adds only the three that are missing; uncheck it and every song goes in, duplicates included. When *every* song in the batch is already there the box would be pointless, so it's dropped and the button reads **Add anyway**. The toast afterwards counts both halves — *Added 3 songs to 'Road Trip' (2 already there)*.

Repeats *inside* your own selection aren't duplicates: two copies of a song the playlist doesn't have are two songs to add. Both add paths run the check — the dialog's Submit and [Quick Add](#quick-add) to the default. The one exception is a conflict that lands while another dialog is open: Nokkvi won't swap the dialog under your cursor, so it toasts *Nothing added…* and leaves the playlist alone.
...
- Reorder tracks with drag-and-drop — same behavior as the [queue](/guides/queue/), down to dropping a track past the last row to append it at the end
- Add tracks by dragging from the Library Browser
- Remove tracks the same way you'd remove them from the queue
- Strip repeats with **Remove Duplicates** — the squares icon in the edit bar, or the same entry in a row's right-click menu. It keeps the first copy of each song and drops every later one across the **whole** playlist: an active search doesn't narrow it, and neither does the row you clicked or your selection. Like every other edit it's staged, so the eyebrow flips to **• UNSAVED** and Save writes it
