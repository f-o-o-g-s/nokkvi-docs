...
| Key | Action |
| :-- | :----- |
| `7` | Switch to Radio view |
| `Enter` | Play the focused station |
| `/` | Focus the search bar |
| `Page Up` | Toggle ascending / descending |
| `R` | Refresh from server |

Multi-select with `Ctrl + Click` and `Shift + Click`. There's only one sort mode (Name), so `Left` / `Right` are no-ops here.

## Search

`/` focuses search. It matches both station **name** and **stream URL** (case-insensitive substring).
