...
Hyprland's [Lua config](https://wiki.hypr.land/Configuring/Lua-API/) is the current binding API. `hl.bind(keys, hl.dsp.exec_cmd(cmd), opts?)` is the canonical shape — the same one the upstream `example/hyprland.lua` uses for multimedia keys. The block below is the author's actual nokkvi binds.
...
-- Volume — { repeating = true } replaces the legacy `binde` repeat flag
...
Add `{ locked = true }` to any bind you want to keep working while the session is locked (the upstream example sets this on `XF86Audio*` keys).
