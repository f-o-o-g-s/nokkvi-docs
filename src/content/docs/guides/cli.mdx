...
| `queue-push` | — | Save the queue to the server — the header Push button. Errors `empty_queue`; `unsupported` if the server lacks the `indexBasedQueue` extension; `unavailable` during radio. See [Queue sync](/guides/queue/#queue-sync) |
| `queue-pull` | — | Restore the server's saved queue without starting playback — the header Pull button. Same guards as `queue-push`, minus the empty-queue check |
