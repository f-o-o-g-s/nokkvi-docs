...
<Tabs syncKey="visualizer-mode">
  ...
  <TabItem label="MilkDrop">
    Plays MilkDrop presets, the classic Winamp feedback-warp visuals, in place of the now-playing cover. See the [MilkDrop guide](/guides/milkdrop/).
  </TabItem>
</Tabs>
...
### LED Mode
[`visualizer.bars.led_bars`](/reference/config/#bars-mode) (default `false`) renders each bar as a stack of LED/VU-meter segments. [`led_segment_height`](/reference/config/#bars-mode) sets the segment height. The gap between segments follows [`bar_spacing`](/reference/config/#bars-mode) (plus `border_width` when borders are on), so the LEDs are spaced vertically the same way the bars are spaced horizontally. Each LED gets its own `border_width` outline, like a bar; when `bar_spacing` is larger than `border_width` the background shows between neighbouring outlines, otherwise they merge. In LED mode the peak indicator is exactly one segment tall, so `peak_height_ratio` is ignored.
...
Over-cover drawing shares its surface with the [lyrics overlay](/guides/lyrics/): with both on, the visualizer keeps playing beneath the words rather than yielding to them.
...
The `milkdrop` mode is different from the others: instead of Nokkvi's own shaders it runs MilkDrop presets through a separate engine, over the cover art on the Queue and Radios panels and full-window in [Theater Mode](/guides/theater-mode/). None of the effects above apply to it. It is cycled after Scope with the `V` hotkey, or set [`visualization_mode = "milkdrop"`](/reference/config/#playback-settings); its own settings are under [`[visualizer.milkdrop]`](/reference/config/#milkdrop-mode). The [MilkDrop guide](/guides/milkdrop/) covers the presets, the keys and the panel menu.
