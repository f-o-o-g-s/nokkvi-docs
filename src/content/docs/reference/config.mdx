...
| `visualizer.scope.line_thickness` | `0.005` | Ring thickness as a fraction of the panel size (0.005 to 0.1). |
| `visualizer.scope.fill_opacity` | `0.75` | Radial gradient fill from the ring toward the center (0.0 = no fill, 1.0 = opaque rim). |
| `visualizer.scope.glow_intensity` | `0.75` | Neon glow halo around the ring (0.0 disables; up to 1.0). |
...
| `visualizer.scope.gradient_mode` | `"static"` | Color mapping mode. Options: `breathing`, `static`, `position`, `height`, `gradient`. |
...
| `visualizer.scope.particle_count` | `512` | Number of particles in the field (0 disables; up to 2048). |
| `visualizer.scope.particle_speed` | `1.0` | Particle launch-speed multiplier — how fast they fly out from the ring (0.1 = lazy drift, 4.0 = energetic). |
...
| `visualizer.scope.echo` | `0.25` | Milkdrop-style zoom/rotate feedback — the ring spirals and tunnels inward, swirling with the bass/beat (0.0 disables; 1.0 = strong persistence). Takes over the display when on. |
{/* config-table:end */}
