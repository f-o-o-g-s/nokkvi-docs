...
| `[visualizer]`, `[visualizer.bars]`, `[visualizer.lines]`, `[visualizer.scope]`, `[visualizer.milkdrop]` | Visualizer settings — the dotted keys in the tables below (e.g. `visualizer.bars.led_bars`) reflect this nesting |
...
| `preview_show_stars` | `true` | Show the star rating column in the smart playlist preview (the rules editor's results pane). |
| `preview_show_love` | `true` | Show the loved/heart column in the smart playlist preview. |
| `preview_show_plays` | `true` | Show the play count column in the smart playlist preview. |
| `preview_show_genre` | `true` | Show the genre column in the smart playlist preview. |
| `preview_show_duration` | `true` | Show the duration column in the smart playlist preview. |
{/* config-table:end */}
...
| `mini_player_show_modes` | `true` | Show the playback-mode toggles (repeat, shuffle, consume, EQ, sound effects, crossfade, bit-perfect, visualizer, lyrics) and their kebab (⋮) overflow in the mini-player bar. Only applies when `track_info_display = "mini_player"`. |
{/* config-table:end */}
...
| Key | Default | Description |
| :--- | :--- | :--- |
| `artwork_auto_max_pct` | `0.70` | Max artwork size in `auto` mode, as a fraction of the window's short axis (0.30–0.70). |
...
| `artwork_cover` | `"show"` | Cover picture or a plain backdrop on the now-playing panels (the Queue cover, the Radios panel, Theater Mode). Options: `show`, `hide_in_theater` (backdrop in Theater Mode only), `hide_everywhere`. The backdrop is black in dark mode and the theme's hard background in light mode; the visualizer and lyrics stay. |
...
Theater Mode (`F11`) is covered in the [Theater Mode guide](/guides/theater-mode/).

{/* config-table:start id="theater" */}
| Key | Default | Description |
| :--- | :--- | :--- |
| `theater_controls` | `"auto_hide"` | How Theater Mode shows the player bar. Options: `auto_hide` (slides away 2.5 seconds after the last mouse or key activity, back on any movement), `always_shown`, `always_hidden` (hotkeys still work). |
| `theater_window_fullscreen` | `false` | When `true`, entering Theater Mode also makes the window fullscreen and leaving restores the previous window mode. When `false`, only the layout changes. |
{/* config-table:end */}
...
| `seek_step_secs` | `5` | Seconds the Seek Backward / Seek Forward keys jump (1 to 60). 1s is a fine nudge; 60s leaps through a long mix. |
{/* config-table:end */}
...
| `visualization_mode` | `"bars"` | Active visualizer mode. Options: `off`, `bars`, `lines`, `scope`, `milkdrop`. Cycle live with the `V` hotkey — see [Visualizer Reference](/reference/visualizer/). |
| `lyrics_enabled` | `false` | Show lyrics over the Queue cover art. Synced lyrics follow the track line by line; plain lyrics drift with playback and scroll with the mouse wheel. Also toggled from the player bar or with the `L` hotkey. Off leaves the cover (and any over-cover visualizer) untouched. See the [Lyrics guide](/guides/lyrics/). |
| `lyrics_fetch_online` | `true` | Fetch missing lyrics from lrclib.net and cache them into the local store for next time. Off resolves lyrics only from your own files, your server, and downloads already cached on disk — no third-party requests. See [Where lyrics come from](/guides/lyrics/#where-lyrics-come-from). |
| `lyrics_backdrop_blur` | `"off"` | Frost the cover art while lyrics display. Options: `off` (the cover stays sharp under the lyrics), `light` (a gentle soften — detail stays readable), `medium` (a true frosted-glass read — shapes survive, detail goes), `heavy` (melts the cover to a color wash). Blurred once per track and cached, never per frame. |
{/* config-table:end */}
...
Per-view sort mode and direction live under `[views]` and persist across restarts. Each view has two keys: `<view>_sort` (string) and `<view>_ascending` (bool). The in-app sort cycle (`Shift + Left` / `Shift + Right` / `Page Up`) writes these for you; manual TOML edits work too.
...
| `visualizer.bars.bar_spacing` | `2` | Gap between bars in pixels. Also sets the gap between LED segments when `led_bars` is on. |
| `visualizer.bars.border_width` | `2.0` | Border thickness in pixels. Outlines each bar and, when `led_bars` is on, each LED segment (0.0 to 5.0). |
...
The `milkdrop` mode plays MilkDrop presets in place of the now-playing cover (and fills [Theater Mode](/guides/theater-mode/)). These keys control how presets change and how sharply they render. See the [MilkDrop guide](/guides/milkdrop/).

{/* config-table:start id="visualizer-milkdrop" */}
| Key | Default | Description |
| :--- | :--- | :--- |
| `visualizer.milkdrop.preset_interval_secs` | `30` | Seconds before MilkDrop picks another preset (0 to 600). `0` keeps the current preset until you press Next Preset. |
| `visualizer.milkdrop.preset_crossfade_secs` | `2.0` | Seconds one preset takes to dissolve into the next (0 to 10, in half seconds). `0` cuts straight to the new preset. A fade never runs longer than half the preset interval, and two presets are drawn at once while it lasts. |
| `visualizer.milkdrop.switch_on_track_change` | `true` | Each new track brings a new preset. Off: presets change only on the timer and the keys. |
| `visualizer.milkdrop.preset_source` | `"all"` | Which presets are drawn: `all` (every preset you haven't hidden), `favorites_only` (your favorites) or `nokkvi` (Nokkvi's own presets, drawn in your theme's colours with the playing cover); all of them while the choice has none left. |
| `visualizer.milkdrop.render_quality` | `"medium"` | Pixels rendered on the shorter side of the panel before scaling: `low` (480), `medium` (720), `high` (1080) or `native` (the panel's own size, the most expensive on a big window). |
| `visualizer.milkdrop.show_preset_names` | `true` | Show each preset's name in a toast when it appears. |
{/* config-table:end */}
