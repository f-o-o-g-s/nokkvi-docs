...
| `volume_normalization` | `false` | Enable EBU R128 loudness normalization. |
| `normalization_level` | `"normal"` | Target loudness. Options: `quiet` (-23 LUFS), `normal` (-14 LUFS), `loud` (-9 LUFS). |
...
<Aside type="note" title="Crossfade and normalization">
  Crossfade and volume normalization can be used together. Normalization adjusts per-track gain; crossfade blends the transition between tracks. If a crossfade sounds uneven in volume, confirm both tracks have normalization metadata in Navidrome (this depends on how your library was tagged).
</Aside>
