...
| `scrollbar_visibility` | `"on_hover"` | How the slot-list scrollbar is shown. Options: `on_hover` (transient fade handle, reserves no width), `always` (permanent track, reserves a right-side gutter), `hidden` (no scrollbar; mouse-wheel still scrolls). |
