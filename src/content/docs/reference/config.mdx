...
| `track_info_display` | `"off"` | Placement of the metadata strip. Options: `off`, `player_bar` (24 px strip below the player bar), `top_bar` (merged into the right side of the nav row in top-nav layout; sits as its own row above the slot list in side-nav / no-nav), `top_bar_under` (its own row directly below the top nav bar; falls back to `top_bar` in side-nav / no-nav layouts), `mini_player` (artwork + title/artist/album column inside the player bar; auto-hides at window widths below ~540 px). |
...
| `strip_separator` | `"dot"` | Character used to join visible fields in the metadata strip. Options: `dot` (·), `bullet` (•), `pipe` (\|), `em_dash` (—), `slash` (/), `bar` (│). |
{/* config-table:end */}
...
| `visualizer.monstercat` | `1` | Monstercat-style energy spread to neighbouring bars. Values below `0.7` snap to `0.0` (disabled); effective range is `0.7` to `1.0`. |
...
| `visualizer.bars.bar_spacing` | `0` | Gap between bars in pixels. |
...
| `visualizer.bars.bar_depth_3d` | `1` | 3D isometric depth in pixels (0.0 disables; up to 20.0). |
{/* config-table:end */}
...
| `visualizer.lines.boat` | `false` | Render a small boat sprite that rides the waveform. |
{/* config-table:end */}
