...
| `lyrics_enabled` | `false` | Show synced lyrics over the Queue cover art, following the playing track line by line. Also toggled from the player bar or with the `L` hotkey. Off leaves the cover (and any over-cover visualizer) untouched. See the [Lyrics guide](/guides/lyrics/). |
| `lyrics_fetch_online` | `true` | Fetch missing lyrics from lrclib.net and cache them into the local store for next time. Off resolves lyrics only from the local store and your server — no third-party requests. See [Where lyrics come from](/guides/lyrics/#where-lyrics-come-from). |
