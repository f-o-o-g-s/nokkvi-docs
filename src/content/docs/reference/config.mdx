...
| `bit_perfect` | `false` | Opt-in bit-perfect output for the Linux/PipeWire backend. Feeds each track to the DAC at its native sample rate and bypasses the EQ, software volume, limiter, and ReplayGain/AGC (volume moves to the PipeWire node). Mutually exclusive with `crossfade_enabled`. Needs PipeWire rate-switching (`allowed-rates`) configured. See the [Audio guide](/guides/audio/#bit-perfect-output). |
