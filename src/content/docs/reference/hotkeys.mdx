...
| Toggle Crossfade | `F` | Enable or disable gapless crossfading between tracks (turns off bit-perfect) |
...
<Aside type="tip">
  To trigger Nokkvi from a WM hotkey (with or without the window focused), use the [`nokkvi` CLI](/guides/cli/) — `nokkvi play-pause`, `nokkvi rate +1`, etc. — bound through your compositor's exec syntax.
</Aside>
