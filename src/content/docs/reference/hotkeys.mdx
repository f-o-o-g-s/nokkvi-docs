...
A direct edit to `config.toml` can also override `escape` or `delete` — the loader accepts those action keys — but the in-app editor refuses to rebind them so you keep a way out of every screen.
