...
| Theater Mode | `F11` | Show only the now-playing cover, visualizer and lyrics; the player bar slides in when the mouse moves. See [Theater Mode](/guides/theater-mode/) |
...
| Cycle Visualizer | `V` | Cycle through visualizer modes: off → bars → lines → scope → MilkDrop |
| Next Preset | `N` | [MilkDrop](/guides/milkdrop/): jump to another preset (only while MilkDrop is playing on screen) |
| Previous Preset | `P` | MilkDrop: go back to the preset before this one |
| Lock Preset | `Shift + M` | MilkDrop: keep the current preset until pressed again |
...
| Toggle Lyrics | `L` | Show or hide [lyrics](/guides/lyrics/) over the Queue cover art |
| Toggle Bit-Perfect | `B` | Cycle bit-perfect output: off → strict → relaxed (turns off crossfade) |
| Seek Backward | `Left` | Jump back by the [Seek Step](/reference/config/#behavior-settings) (5s default, 1–60s). Hold to scrub — or step the focused [Trawl](/guides/trawl/#editing-the-controls-from-the-keyboard) control to its previous value |
| Seek Forward | `Right` | Jump ahead by the Seek Step. Hold to scrub — or step the focused Trawl control to its next value |
...
| Action | Shortcut | Description |
| :--- | :--- | :--- |
| Previous Sort Mode | `Shift + Left` | Cycle the sort mode backward — or step the focused [Trawl](/guides/trawl/#editing-the-controls-from-the-keyboard) control to its previous value |
| Next Sort Mode | `Shift + Right` | Cycle the sort mode forward — or step the focused Trawl control to its next value |
...
In [Theater Mode](/guides/theater-mode/#keys), `Escape` leaves it. Keys that act on the hidden list (Tab, Enter, ratings, queue edits) leave it too without acting, and view keys leave it and then switch.
...
### When two actions share a key

Rebinding in the Settings UI can't create a shared key: pressing a combo another action already holds **swaps** the two, and the row confirms it — *Swapped with Seek Forward*. Nothing is left double-bound.

Hand-editing `config.toml` can, and then Nokkvi picks one owner in a fixed order — the same on every launch, and the same answer the conflict check gives:

1. A **reserved** action (`escape`, `reset_to_default`) always wins, so nothing parked on one can shadow your way out of a screen.
2. Otherwise, an action **moved off its default** wins over one merely sitting where it shipped — you put it there on purpose.
3. Otherwise, the action declared first wins.

Loading a config with a shared combo logs one warning per combo naming every claimant and the winner, so `nokkvi.log` tells you which one is live.
...
| Next Preset | `next_visualizer_preset` |
| Previous Preset | `previous_visualizer_preset` |
| Lock Preset | `toggle_visualizer_preset_lock` |
